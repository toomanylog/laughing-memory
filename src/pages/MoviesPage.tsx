...
const MoviesPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function loadMovies() {
      ...
      try {
        ...
        const result = await getContentsByType('movie');
        
        if (isMounted) {
          if (result && result.length > 0) {
            console.log(`${result.length} films chargés avec succès`);
            setMovies(result);
          } else {
            console.warn("Aucun film trouvé");
            setError("Aucun film n'est disponible pour le moment.");
          }
        }
      } catch (err) {
        if (isMounted) {
          console.error("Erreur lors du chargement des films:", err);
          setError("Une erreur est survenue lors du chargement des films. Veuillez réessayer.");
        }
      } finally {
        if (isMounted) {
          setLoading(false);
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      {!loading && !globalLoading && !error && !globalError && movies.length === 0 && (
      ...
      {!loading && !globalLoading && movies.length > 0 && (
        <ContentGrid>
          {movies.map(movie => (
